...
- [x] **聊天体验（已完成）**: 流式输出（Rust SSE → Tauri Channel）、Markdown 渲染、停止生成、逐条复制、清空、按文档持久化、导出 Markdown。
- [x] **阅读器（已完成）**: 缩放/适宽/页码跳转、文档内搜索高亮、虚拟滚动（大 PDF 不再整本渲染）、拖拽打开、二进制 IPC 传输。
- [x] **界面（已完成）**: 亮/暗主题、设置抽屉（凭据移出主工具栏）、可拖拽三栏、lucide 图标、全新应用图标、消息/抽屉动画。
- [x] **智能（已完成）**: 打开自动摘要（**默认关**，会耗 AI 额度需在设置开启）、选中文字浮动「在 Chat 2 提问」、长文档本地 RAG 检索（**默认关**，本地嵌入模型 ~25MB 首次下载、不耗云额度）。
- [ ] **进一步打磨**: 本地模型的 ONNX runtime WASM 与权重目前首次运行需联网拉取，若要真正纯离线可考虑随包内置；搜索高亮目前限于 pdf.js 单 span 内匹配。
